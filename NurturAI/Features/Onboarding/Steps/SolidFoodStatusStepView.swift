...
struct SolidFoodStatusStepView: View {
    @Binding var stepState: SolidFoodStatus

    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            VStack(alignment: .leading, spacing: 8) {
                Text(Strings.Onboarding.SolidFoods.heading)
                    .font(NurturTypography.title2)
                    .foregroundStyle(NurturColors.textPrimary)
                Text(Strings.Onboarding.SolidFoods.subheading)
                    .font(NurturTypography.subheadline)
                    .foregroundStyle(NurturColors.textSecondary)
            }

            VStack(spacing: 12) {
                ForEach(SolidFoodStatus.allCases, id: \.self) { selectedAnswer in
                    Button {
